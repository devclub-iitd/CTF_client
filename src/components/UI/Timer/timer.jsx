...
const calculateStateFromProps = (props) => {
  const { dateTo, numberOfFigures, mostSignificantFigure } = props;
...
class Timer extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    const newState = calculateStateFromProps(props);
    return newState;
  }
  ...
  animateAndChangeIfNeeded = (unit, prevUnit) => {
    const { speed, significance } = this.state;

    if (significance.indexOf(unit) !== -1) {
      const unitSpan = this.refs[unit];
      const digits = unitSpan.children;
  ...
  render() {
    const { significance } = this.state;
    const { locales, locales_plural } = this.props;
