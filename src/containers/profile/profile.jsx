  ...
  render() {
    ...
    if (profile) {
      compList = profile.participant.map((el) => {
        ...
        const { _id } = el;
        return (
          <div className={classes.list} key={_id}>
            <Link
              style={{ textDecoration: "none", color: "rgba(245,0,87,1)" }}
              to={{
            ...
            >
              <div className={classes.title} role="presentation">
                <div className={classes.innerListTitle}>{el.eventName}</div>
              </div>
            </Link>
          </div>
        );
    ...
    if (profile) {
      display = (
        <div className={classes.mainCont}>
          <div className={classes.pageTitle}>{profile.name}</div>
          <div className={classes.miniLine} />
          <div className={classes.subDetail}>
            <span className={classes.subDetailTitle}>User Name:</span>{" "}
            <span className={classes.subDetailDesc}>{profile.username}</span>
            <br /> <br />
            <span className={classes.subDetailTitle}>Email:</span>{" "}
            <span className={classes.subDetailDesc}>{profile.email}</span>
            <br /> <br />
            <span className={classes.subDetailTitle}>Phone No:</span>{" "}
            <span className={classes.subDetailDesc}>{profile.number}</span>
            <div className={classes.miniLine} />
          </div>
          <Box p={4}>
            <div className={classes.subTitle}>Competitions Participated</div>
            <div className={classes.miniLine} />
          </Box>
          <div className={classes.compListCont}>{compList}</div>
        </div>
      );
    }
...
const mapDispatchToProps = (dispatch) => ({
  onInitProfile: (userId, token) =>
    dispatch(ProfileActions.initProfile(userId, token)),
});
